	...
	if ( ! isset( $_GET[ 'infinity' ] ) ) {
		...
		if ( $total_pages > 1 ) {
			...
			$arguments = apply_filters( 'odin_pagination_args', array(
					...
					'next_text' => __( 'Proximo', 'odin' ),
				)
			);

			$pagination = '<nav class="pagination-wrap" aria-label="Paginacao">' . paginate_links( $arguments ) . '</nav>';
...
function odin_breadcrumbs( $homepage = '' ) {
	...
	if ( ! is_home() && ! is_front_page() || is_paged() ) {
		...
		// First level.
		echo '<nav class="nav-breadcrumb" style="--bs-breadcrumb-divider: \'>\';" aria-label="breadcrumb">'; 
			...
			if ( 'post' != $post->post_type ) {
				...
				if ( $category->parent ) {
					if ( $parent_cat->parent ) {
						echo '<li class="breadcrumb-item"><a href="' . get_term_link( $top_cat, 'category' ) . '">' . $top_cat . '</a></li>';
					}
					echo '<li class="breadcrumb-item"><a href="' . get_term_link( $parent_cat ) . '">' . $parent_cat->name. '</a></li>';
				}
		...
		// Page with parents.
			...
			while ( $parent_id ) {
				...
				$breadcrumbs[] = '<li class="class="breadcrumb-item"><a href="' . esc_url( get_permalink( $page->ID ) ) . '">' . get_the_title( $page->ID ) . '</a></li>';
		...
		// Category archive.
			...
			if ( 0 != $current_category->parent ) {
				...
				$parents = str_replace( '<a', '<li class="breadcrumb-item"><a', $parents );
		...
		// Archives per days.
			...
			echo '<li class="class="breadcrumb-item"><a href="' . get_year_link( get_the_time( 'Y' ) ) . '">' . get_the_time( 'Y' ) . '</a></li>';

			echo '<li class="class="breadcrumb-item"><a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '">' . get_the_time( 'F' ) . '</a></li>';
		...
		// Archive fallback for custom taxonomies.
			...
			if ( ! empty( $taxonomy->object_type ) ) {
				...
				if ( is_woocommerce() ) {
					$shop_page    = get_post( wc_get_page_id( 'shop' ) );
					echo '<li class="class="breadcrumb-item"><a href="' . esc_url( get_permalink( $shop_page ) ) . '">' . get_the_title( $shop_page ) . '</a></li>';
				} else {
					$_post_type = array_shift( $taxonomy->object_type );
					$post_type = get_post_type_object( $_post_type );
					echo '<li class="class="breadcrumb-item"><a href="' . get_post_type_archive_link( $post_type->name ) . '">' . $post_type->label . '</a></li> ';
				}
			}
			...
			if ( 0 != $current_object->parent ) {
				...
				echo '<li class="class="breadcrumb-item"><a href="' . get_term_link( $parent_term ) . '">' . $parent_term->name . '</a></li>';
			}
